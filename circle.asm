...
mac $a0, $a0, $a0, $zero, 0, 0                            # $a0 = r^2 (squared radius)
add $t0, $imm1, $zero, $zero, -1, 0						            # $t0 = -1 (first pixel, will be moved to 0 in CALC)
sll $s0, $imm1, $imm2, $zero, 1, 16                       # $s0 = 65536 (max index of pixels)

CALC:
bge $zero, $t0, $s0, $imm2, 0, RIP				                # if $t0 >= $s0 then RIP 
add $t0, $t0, $imm1, $zero, 1, 0								          # $t0 += 1 (move to the next pixel)
...
srl $t2, $t0, $imm1, $zero, 8, 0								          # $t2 = $t0 : 256 rounded down = y
sub $t2, $t2, $imm1, $zero, 128, 0                        # $t2 = $t2 - 128 = y - 128
...
COLOR:
out $zero, $imm1, $zero, $t0, 20, 0							          # montioraddr = $t0 (pixel index)
